image frag1 = im.Scale("wth/frag1.png", 1920, 1080)
image frag2 = im.Scale("wth/frag2.png", 1920, 1080)
image frag3 = im.Scale("wth/frag3.webp", 1920, 1080)
image frag4 = im.Scale("wth/frag4.png", 1920, 1080)
image tatarifrag = im.Scale("wth/tatari fragment.webp", 1920, 1080)
image tatari = im.Scale("wth/tatari.png", 1920, 1080)

image dis = im.Scale("sprites/disembodied.png", 681, 1080)
...
label wth3:
    ...
    witch "Ahahahahaahahah. {p}Il tuo aspetto. {w} Troppo ridicolo hehehehahaa."
    ...
    menu:
        ...
        "QUANDO RIVEDO LARRY LO LICENZIO!!!":
            witch "Placati, ciò che è successo non è interamente colpa del ragazzo.{w} É il fato che ti riporta inevitabilmente qui."

    hide dis

    witch "Capisco la tua confusione.{w} Stai pur certo, ciò che hai visto non è un altro mondo. Hai semplicemente potuto osservare l’unica e sola Hinamizawa da due prospettive diverse."

    scene frag2 with dissolve

    pause 1

    scene frag3 with dissolve

    pause 1

    scene frag4 with dissolve
    ...
    menu:
        ...
        "Si però parlami con rispetto":
            witch "Sei 100 anni in anticipo per meritare la mia cortesia, giovanotto. Ora rispondi come si conviene."
            ...
            menu:
                ...
                "Non mi piace la tua attitudine.":
                    witch "Mhhh. proviamo in questo modo."
    ...
    witch "Allora, come ti senti dopo esser stato investito da cotanta saggezza?"
    ...
    menu:
        "Grazie delle dritte, voce incorporea senza nome.":
            ...
            witch "Guarda un po' tu chi parla..."
            witch "Se per la prossima volta che farai ritorno qui avrai anche degli occhi, potrei pure decidere di mostrare a te la mia forma, lasciamo le presentazioni per allora."
            hide dis

        "Sinceramente... sono ancora confuso.":
            hide dis
            witch "Abbiamo stretto un patto, cerca di non deludermi.{w} Credevo che nessuna richiesta fosse troppo per il capo del BKG.{w} Mi sbaglio?"

    witch "Ordunque, qualunque sia il destino che ti attende è arrivato il momento di andargli in contro."
    ...
    witch "Va adesso piccola serpe nel pozzo, mettiti in gioco e lasciami scoprire,{w} se sarai solo un altro pedone{w}.{w}.{w}."

    stop music

    play sound "audio/zbiin.ogg"

    witch "O se sarai colui che fa girare la scacchiera."
...
label sissignora:
    ...
    witch "Da adesso {i}{color=#0000cf}Tutto ciò che dirò in blu sarà una verità che trascende i frammenti.{/color}{/i}"
    ...
    pause 2
    witch """Tre domande e tre incontrovertibili verità, queste erano le regole. Non dirmi che non sei soddisfatto.
    ...
    menu:
        "Chi era il \"responsabile\" chiamato a casa di Keiichi da Mion e Rena?":
            pass
    witch "Non hai prestato attenzione? Lo hanno detto pure le ragazze{w}: il responsabile è il responsabile, come il regista di un film."

    play sound "audio/truth.mp3"
    witch "In ogni caso... {i}{color=#0000cf}Lo incontrerai presto.{/color}{/i} Questo te lo posso garantire."
...
label oyashiro_question:
    witch "Sapevo che lo avresti chiesto, è una teoria più che lecita.{w} Quasi mi dispiace doverla frantumare."
    play sound "audio/truth.mp3"
    witch "{i}{color=#0000cf}Non rispondo a quel nome, nè sono mai stata chiamata con esso fino ad ora.{/color}{/i}"
    witch "Che peccato, la risposta più semplice non è necessariamente quella giusta."
    witch "Se avessi detto di si mi avresti incolpata per tutte le tragedie come il resto di voi umani fa con quell'essere?"
    return

label mion_question:
    witch "Haha, scelta interessante, allora ascolta con attenzione il significato di queste parole."
    play sound "audio/truth.mp3"
    witch "{i}{color=#0000cf}La ragazza battezzata Mion Sonozaki non era vittima di alcuna possessione demoniaca{w}, tuttavia ella era convinta di quel che diceva.{/color}{/i}"
    play sound "audio/laugh.mp3"
    pause 1
    return

label keiichi_question:
    witch "Capelli chiari?{w} ...oh, giusto, è come tu e il tuo assistente chiamate Maebara Keiichi."
    play sound "audio/truth.mp3"
    witch "{i}{color=#0000cf}Nonostante Keiichi non abbia visto o colpito nulla, c'era senza dubbio qualcuno o qualcosa immediatamente dietro di lui.{/color}{/i}"
    witch "Mentre per quanto riguarda il resto delle sue sventure... huhu chissà."
    return
